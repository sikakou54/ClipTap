...
### Google Play Billingのテスト購入について

**Androidでサブスクリプションをテストする際、期間が短縮されます**：
...
**これは正常な動作です**。テスト購入を素早く検証するためのGoogle Play Billingの仕様です。

- テスト環境では、5分ごとに更新通知が来ます
- 本番環境（Production）では、正常な期間（1ヶ月、1年）で動作します
- テストアカウントを使用している場合、この短縮期間が適用されます

参考: https://developer.android.com/google/play/billing/test

### iOSのサブスクリプションテスト

- Sandbox環境では、以下の短縮期間が適用されます：
  - 1週間 → 3分
  - 1ヶ月 → 5分
  - 2ヶ月 → 10分
  - 3ヶ月 → 15分
