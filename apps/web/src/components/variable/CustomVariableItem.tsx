...
export function CustomVariableItem({ variable, value, isLast, onEdit, onDelete }: CustomVariableItemProps) {
  ...
  return (
    ...
    >
      {/* 変数アイコン */}
      <div className={`p-2 rounded-lg ${isEnabled ? 'bg-purple-100 dark:bg-purple-900/30' : 'bg-gray-100 dark:bg-[#2A2A2A]'}`}>
        <svg
          className={`w-5 h-5 ${isEnabled ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-[#707070]'}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
