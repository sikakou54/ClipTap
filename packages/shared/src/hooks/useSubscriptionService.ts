 ...
 * - プラン一覧の取得・planId指定の購入・SubscriptionStatus（プラン種別・有効期限）が必要な画面
 *   （ペイウォール／サブスク管理）はこのフックを使う。
 *
 * Provider版もプラン種別・有効期限・プラン一覧・購入を公開しているが、
 * getOfferings / purchasePackage は RevenueCat のパッケージを unknown 型のまま扱う。
 * 型付きの SubscriptionPlan / SubscriptionStatus と planId 指定の購入が要る場合はこちらを使う。
 */
