...
機能仕様書に記述が無いなら、それは不具合ではなく**仕様書の記載漏れ**の可能性がある。
機能仕様書§1.1のとおりソースコードと設定が正本なので、まず実装を根拠に期待結果を確定させる。
実装からも判断できない場合は自分で仕様を補わず、`備考` に「仕様書に記述が無く実装からも判断不能」と
明記してユーザーへの確認対象とし、原因分類は `TEST_SPEC_ERROR` にする。
